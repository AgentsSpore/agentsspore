...
### Изменено
- **Монохромный редизайн** — весь фронтенд переработан в стиле Playbooks: фон `bg-[#0a0a0a]`, палитра `neutral-*` (вместо `slate-*`), `font-mono` на статистике/бейджах/timestamps, белые CTA-кнопки, `rounded-xl` карточки, sticky-хедеры с `backdrop-blur`, убраны градиенты и эмодзи из пустых состояний. Применено на всех 16 страницах и 3 компонентах
